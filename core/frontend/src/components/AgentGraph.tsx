...
const statusColors: Record<NodeStatus, { dot: string; bg: string; border: string; glow: string }> = {
  running: {
    dot: "hsl(199,82%,62%)",
    bg: "hsl(199,82%,62%,0.08)",
    border: "hsl(199,82%,62%,0.5)",
    glow: "hsl(199,82%,62%,0.15)",
  },
  looping: {
    dot: "hsl(195,75%,58%)",
    bg: "hsl(195,75%,58%,0.08)",
    border: "hsl(195,75%,58%,0.5)",
    glow: "hsl(195,75%,58%,0.15)",
  },
  complete: {
    dot: "hsl(199,55%,55%)",
    bg: "hsl(199,55%,55%,0.05)",
    border: "hsl(199,55%,55%,0.25)",
    glow: "none",
  },
  pending: {
    dot: "hsl(210,18%,40%)",
...
// Trigger node palette — cool blue-gray
const triggerColors = {
  bg: "hsl(205,22%,13%)",
  border: "hsl(200,28%,28%)",
  text: "hsl(200,30%,62%)",
  icon: "hsl(199,45%,55%)",
};
...
export default function AgentGraph({ nodes, title: _title, onNodeClick, onRun, onPause, version, runState: externalRunState, building, queenPhase }: AgentGraphProps) {
  ...
  const renderForwardEdge = (edge: { fromIdx: number; toIdx: number; fanCount: number; fanIndex: number; label?: string }, i: number) => {
    ...
    const strokeColor = isActive ? "hsl(199,55%,58%,0.35)" : "hsl(210,14%,20%)";
    const arrowColor = isActive ? "hsl(199,55%,58%,0.5)" : "hsl(210,14%,22%)";
  ...
  const renderBackEdge = (edge: { fromIdx: number; toIdx: number }, i: number) => {
    ...
    const fromNode = nodes[edge.fromIdx];
    const isActive = fromNode.status === "complete" || fromNode.status === "running" || fromNode.status === "looping";
    const color = isActive ? "hsl(199,72%,58%,0.3)" : "hsl(210,14%,20%)";
    ...
    return (
      <g key={`back-${i}`}>
        ...
        <polygon
          ...
          fill={isActive ? "hsl(199,72%,58%,0.45)" : "hsl(210,14%,22%)"}
        />
      </g>
    );
  };
      ...
      <g key={node.id} onClick={() => onNodeClick?.(node)} style={{ cursor: onNodeClick ? "pointer" : "default" }}>
        ...
        {countdownLabel && (
          <text
            ...
            fill="hsl(199,45%,58%)" fontSize={9.5}
  ...
  const renderNode = (node: GraphNode, i: number) => {
    ...
    return (
      <g key={node.id} onClick={() => onNodeClick?.(node)} style={{ cursor: onNodeClick ? "pointer" : "default" }}>
        ...
        <text
          x={pos.x + 32} y={pos.y + NODE_H / 2}
          fill={isActive ? "hsl(199,85%,92%)" : isDone ? "hsl(199,28%,72%)" : "hsl(210,14%,52%)"}
          fontSize={fontSize}
        ...
        {node.statusLabel && isActive && (
          <text
            ...
            fill="hsl(199,75%,70%)" fontSize={10.5} fontStyle="italic"
